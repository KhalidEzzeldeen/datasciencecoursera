...
trainX <- read.table("data\\UCI HAR Dataset\\train\\X_train.txt")
testX  <- read.table("data\\UCI HAR Dataset\\test\\X_test.txt")
dataX  <- rbind(trainX, testX)

# Read and bind S training and testing

trainS <- read.table("data\\UCI HAR Dataset\\train\\subject_train.txt")
testS  <- read.table("data\\UCI HAR Dataset\\test\\subject_test.txt")
dataS  <- rbind(trainS, testS)

# Read and bind Y training and testing

trainY <- read.table("data\\UCI HAR Dataset\\train\\y_train.txt")
testY  <- read.table("data\\UCI HAR Dataset\\test\\y_test.txt")
...
write.table(allData, "data\\UCI HAR Dataset\\allData.txt")
...
write.table(dataResult, "data\\UCI HAR Dataset\\allDataMeanAvg.txt")
